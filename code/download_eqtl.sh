...
for i in "${!study_ids[@]}"; do
    ...
    # Credible intervals:
    #curl -C - -s -o "data/eqtl/${dataset_name}_credible_sets.tsv.gz" \
    #    "https://ftp.ebi.ac.uk/pub/databases/spot/eQTL/susie/$study_id/$dataset_id/$dataset_id.credible_sets.tsv.gz"
    
    # LBF:
    curl -C - -s -o "data/eqtl/${dataset_name}_lbf_variable.txt.gz" \
        "https://ftp.ebi.ac.uk/pub/databases/spot/eQTL/susie/$study_id/$dataset_id/$dataset_id.lbf_variable.txt.gz"
done
